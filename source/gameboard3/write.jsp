...
<%
	...
	conn=DriverManager.getConnection(jdbc_url,"root","1234");
	 ...
	 try{
	String sql="select GAME1, GAME2, GAME3, GAME4, GAME5, GAME6 from data where ID=?";
	pstmt=conn.prepareStatement(sql);
	pstmt.setString(1, cid);
	ResultSet rs=pstmt.executeQuery();
	int game1, game2, game3, game4, game5, game6;
	int avg;
	while(rs.next()){
	%>
	<%game1=rs.getInt(1);%>
	...
	<%game6=rs.getInt(6);%>
	<%
		avg=game1+game2+game3+game4+game5+game6;
		avg=avg/6;
		//out.println(avg);
		if(avg<=5000){
			out.print("<script>");
			out.print("alert('게임 평균점수가 5000점을 넘지 못하여 접근 불가합니다.');");
