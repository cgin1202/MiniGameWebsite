<%@ page contentType="text/html; charset=utf-8" pageEncoding="utf-8" import="java.sql.*,java.util.*"%>
		...
		String sql2="SELECT GAME1 FROM data where ID=?";
		pstmt2=conn.prepareStatement(sql2);
		pstmt2.setString(1,id);
		ResultSet rs=pstmt2.executeQuery();
		while(rs.next()){
			datagamescore1=rs.getInt("GAME1");
		}
		
		
		if(gamescore1>datagamescore1){
			String sql="UPDATE data SET GAME1 =? where ID=?";
		...
		out.print("<script>");
		out.println("location.href='1945_game.jsp';");
		out.print("</script>");
	 %>